...
struct server::impl {
    ...
    void start_accept() {
        acceptor_.async_accept(socket_, [this](std::error_code ec) {
            if (!ec) {
                RPCLIB_LOG_INFO("Accepted connection.");
                ...
                RPCLIB_LOG_ERROR("Error while accepting connection: {}", ec);
            }
            start_accept();
...
server::server(uint16_t port)
    ...
    RPCLIB_LOG_INFO("Created server on localhost:{}", port);
    pimpl->start_accept();
}
...
server::server(std::string const &address, uint16_t port)
    ...
    RPCLIB_LOG_INFO("Created server on address {}:{}", address, port);
    pimpl->start_accept();
}
...
void server::async_run(std::size_t worker_threads) {
    pimpl->loop_workers_.create_threads(worker_threads, [this]() {
        name_thread("server");
        RPCLIB_LOG_INFO("Starting");
        pimpl->io_.run();
        RPCLIB_LOG_INFO("Exiting");
    });
}
